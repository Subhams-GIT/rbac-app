...
export const SignIn = ({ userType, onSwitchForm, showToast }:sigIn) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false);
    showToast(`${userType === 'admin' ? 'Admin' : 'User'} signed in successfully`, 'success');
    ...
    >
      <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6">
        <div className="flex items-center justify-center mb-4">
          ...
          >
            <span className="text-white text-lg font-bold">{userType === 'admin' ? 'A' : 'U'}</span>
          </motion.div>
        </div>
        ...
        <p className="text-center text-gray-600 text-xs mb-5">Welcome back! Please sign in to continue</p>
        ...
        <p className="text-center text-gray-600 text-xs mt-4">
          ...
          <button onClick={onSwitchForm} className="text-black font-semibold hover:underline">
            Sign Up
          </button>
        </p>
      </div>
